# train_ablation_WF.py — WEF-YOLO 消融变体 WF (LWDS stem + WRF neck)
# 模型: yolo11s-ablation-WF.yaml (scale s); 数据集: SSDD_official(928/232)
# 基线 = 加 P2 去 P5 的 YOLO11s;三模块 W=LWDS(stem) E=SFEE(P4) F=WRF(neck)。
# 训练配方与 ssdd_WEF 完全一致,仅模型不同 → 同口径消融。
