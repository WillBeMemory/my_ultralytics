...
MODEL_NAME = "yolo11n-dpconv-moe.yaml"
...
def train_model():
    ...
    def on_train_batch_end(trainer):
        ...
        for name, module in model.named_modules():
            if module.__class__.__name__ == 'DPConv' and hasattr(module, 'current_ratio'):
                ratio = module.current_ratio.item()
                ratio_sum[name] += ratio
                ratio_count[name] += 1

    def on_train_epoch_end(trainer):
        print("\n========== DPConv Selection Ratios ==========")
        for name in ratio_sum:
            avg = ratio_sum[name] / ratio_count[name]
            print(f"  {name}: {avg:.4f}")
        print("=============================================\n")
        ratio_sum.clear()
        ratio_count.clear()
