...
MODEL_NAME = "yolo11n-gatedc3k2-hipa.yaml"
...
def train_model():
    ...
    def on_train_epoch_end(trainer):
        """每个 epoch 结束后打印 GatedC3k2 门控分配"""
        epoch = trainer.epoch
        model = trainer.model

        print(f"\n========== Epoch {epoch} GatedC3k2 Gate Allocation ==========")
        for name, module in model.named_modules():
            if hasattr(module, 'get_gate_probs'):
                probs = module.get_gate_probs().cpu().numpy()
                print(f"  {name}: m_path={probs[0]:.4f}, identity_path={probs[1]:.4f}")
        print("===============================================================\n")
