...
def train_model():
    ...
    def log_mask_ratio(trainer):
        for module in trainer.model.model.modules():
            if hasattr(module, 'last_mask') and module.last_mask is not None:
                ratio = module.last_mask.mean().item()
                print(f"Epoch {trainer.epoch + 1}: mask keep ratio = {ratio:.4f}")

    model.add_callback('on_train_epoch_end', log_mask_ratio)
