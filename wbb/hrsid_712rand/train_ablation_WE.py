# train_ablation_WE.py — WEF-YOLO 消融变体 WE (+LWDS +SFEE)
# 模型: yolo11s-ablation-WE.yaml (scale s); 数据集: HRSID 712rand(主划分,3923/560/1121)
# 基线 = 加 P2 去 P5 的 YOLO11s;三模块 W=LWDS(stem) E=SFEE(P4) F=WRF(neck)。
# 训练配方与 Ours(yolo11s-test.yaml)完全一致,仅模型不同 → 同口径消融。

from ultralytics import YOLO
import os
import torch

MODEL_NAME = "yolo11s-ablation-WE.yaml"
DATASET_PATH = "../cfg/hrsid_712rand.yaml"
RUN_NAME = "ablation_WE"
