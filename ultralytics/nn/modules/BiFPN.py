...
from ultralytics.nn.modules.DepthwiseSeparableConv import DepthwiseSeparableConvWithWTConv2d  # 导入小波卷积


class BiFPN(nn.Module):
    """
    YOLO11 默认 Neck 的复刻版，自底向上路径使用深度可分离小波卷积下采样。
    ...
    参数:
        ...
        wt_depth    : 小波卷积的递归深度，默认 2
    """

    def __init__(self, channels, out_channels=None, c3k2_kwargs=None, wt_depth=1):
        super().__init__()
        c3, c4, c5 = channels
        ...
        # ---- 自底向上（使用小波卷积下采样） ----
        # 替换原来的 Conv(o3, o4, k=3, s=2) 和 Conv(o4, o5, k=3, s=2)
        self.down1 = DepthwiseSeparableConvWithWTConv2d(o3, o4, k=3, s=2, wt_depth=wt_depth)
        self.down2 = DepthwiseSeparableConvWithWTConv2d(o4, o5, k=3, s=2, wt_depth=wt_depth)

        self.p4_out_c3k2 = C3k2(o4 + o4, o4, **kwa)  # 下采样后通道已对齐
        self.p5_out_c3k2 = C3k2(c5 + o5, o5, **kwa)  # 注意：c5 是原始 P5 输入通道
    ...
    def forward(self, features):
        ...
        # ---- 自底向上 ----
        p3_down = self.down1(p3_td)  # 小波卷积下采样，o3 -> o4，尺寸减半
        p4_out = self.p4_out_c3k2(torch.cat([p4_td, p3_down], dim=1))

        p4_down = self.down2(p4_out)  # o4 -> o5，尺寸减半
