...
class BiFPN(nn.Module):
    """
    BiFPN 完整实现 (EfficientDet)，支持自定义输出通道数。
    参数:
    ...
    def __init__(self, channels, out_channels=None, use_depthwise=True):
        ...
        if out_channels is None:
            o3, o4, o5 = c3 // 2, c4, c5   # 默认目标通道
        else:
            o3, o4, o5 = out_channels

        # ---- 通道投影 (1x1 Conv) ----
        self.p3_proj = nn.Conv2d(c3, o3, 1, bias=False)      # P3 -> o3 (新增)
        self.p5_to_p4 = nn.Conv2d(c5, o4, 1, bias=False)    # P5 -> o4
        self.p4_to_p3 = nn.Conv2d(o4, o3, 1, bias=False)    # P4_td_up -> o3
        self.p3_to_p4 = nn.Conv2d(o3, o4, 1, bias=False)    # P3_down -> o4
        self.p4_to_p5 = nn.Conv2d(o4, o5, 1, bias=False)    # P4_down -> o5

        # ---- 加权融合节点 ----
        self.p4_td_fuse = BiFPN_Add(2)  # P4 + P5_up
        self.p3_td_fuse = BiFPN_Add(2)  # P3_proj + P4_td_up_proj
        self.p4_bu_fuse = BiFPN_Add(2)  # P4_td + P3_down_proj
        self.p5_bu_fuse = BiFPN_Add(2)  # P5 + P4_down_proj
